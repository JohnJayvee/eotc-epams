  ...
  <div class="col-md-12">
    <h4 class="pb-4">Record Data
      <span class="float-right">
        <a href="{{route('system.service.create')}}" class="btn btn-sm btn-primary">Add New</a>
      </span>
    </h4>
    <div class="table-responsive shadow fs-15">
      <table class="table table-striped">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $service->name}}</td>
            <td>{{ Helper::date_format($service->created_at)}}</td>
            <td >
              <button type="button" class="btn btn-sm p-0" data-toggle="dropdown" style="background-color: transparent;"> <i class="mdi mdi-dots-horizontal" style="font-size: 30px"></i></button>
              <div class="dropdown-menu" aria-labelledby="dropdownMenuSplitButton2">
                <a class="dropdown-item" href="{{route('system.service.edit',[$service->id])}}">Edit Service Type</a>
                <a class="dropdown-item action-delete"  data-url="{{route('system.service.destroy',[$service->id])}}" data-toggle="modal" data-target="#confirm-delete">Remove Record</a>
              </div>
            </td>
          </tr>
          @empty
          <tr>
           <td colspan="5" class="text-center"><i>No Service Type Records Available.</i></td>
          </tr>
          @endforelse
    ...
    @if($services->total() > 0)
    <nav class="mt-2">
      <p>Showing <strong>{{$services->firstItem()}}</strong> to <strong>{{$services->lastItem()}}</strong> of <strong>{{$services->total()}}</strong> entries</p>
