...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{route('system.business_transaction.approved')}}">Transaction Management</a></li>
...
<div class="col-md-6 grid-margin stretch-card">
  <div class="card">
    <div class="card-body">
      <h4 class="card-title mb-3">Business Transaction Upload Form</h4>
      <form class="create-form" method="POST" enctype="multipart/form-data" id="upload_form">
        @include('system._components.notifications')
        {!!csrf_field()!!}
        <div class="row">
          <div class="col-md-4">
            <p class="text-title fw-500">Company: <span class="text-black">{{ $business_transaction->business ? $business_transaction->business->company_name : ""}}</span></p>
          </div>
          <div class="col-md-4">
            <p class="text-title fw-500">Service Type: <span class="text-black">{{$business_transaction->service ? Str::title($business_transaction->service->name) : "N/A"}}  </span></p>
          </div>
          <div class="col-md-4">
            <p class="text-title fw-500">Perrmit Type: <span class="text-black">{{$business_transaction->permit ? Str::title($business_transaction->permit->name) : "N/A"}}  </span></p>
          </div>
        </div>
        ...
        <a href="{{route('system.business_transaction.show',[$business_transaction->id])}}" class="btn btn-light">Return to Transaction</a>
      </form>
    </div>
  </div>
</div>
